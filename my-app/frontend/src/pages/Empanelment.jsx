                      ...
                      {products.map((p,i)=>(
                        <tr key={p.id||i} style={{borderBottom:`1px solid ${pTd(d).borderBottomColor}`}}>
                          ...
                          <td style={{...pTd(d),padding:"9px 14px"}}>
                            <span className={`chip ${p.onboarding_process==="online"?"chip-yes":"chip-no"}`}>{p.onboarding_process?p.onboarding_process.charAt(0).toUpperCase()+p.onboarding_process.slice(1):"—"}</span>
                          </td>
...
export default function Empanelment({ inline=false, onDataChange, initialTab, theme="dark" }) {
  ...
  // ── LIVE COUNTS: always sourced from their respective tables, not from DB ──
  const [liveProducts, setLiveProducts] = useState({}); // { AMC_name: count } from products table
  const [liveBoarding, setLiveBoarding] = useState({}); // { AMC_name: count } from invested/completed
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (loaded.length > 0) {
        // Fetch BOTH sources in parallel and build live count maps
        try {
          ...
          // Count by amc_name (lowercase key)
          const prodMap = {};
          ...
          // Map back with original AMC_name casing from empanelment rows
          const liveP = {}, liveB = {};
          loaded.forEach(row => {
  ...
  /* ── Render cell: always use live count for products & boardings ── */
...
const EMP_CSS = `
  ...
  .tbl-wrap::-webkit-scrollbar{height:5px}.tbl-wrap::-webkit-scrollbar-thumb{background:rgba(79,142,247,.3);border-radius:4px}
  .fd-tbl { width:100%;border-collapse:collapse;font-size:.8rem; }
  .fd-tbl thead tr { border-bottom:1px solid rgba(79,142,247,.15); }
  .fd-tbl th { padding:9px 14px;text-align:left;font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:rgba(79,142,247,.65);font-family:var(--fh,'Orbitron',sans-serif);white-space:nowrap; }
  ...
  .add-btn { display:inline-flex;align-items:center;gap:6px;padding:7px 16px;border-radius:20px;border:none;color:#fff;font-size:.78rem;font-family:var(--fb,'Exo 2',sans-serif);font-weight:700;cursor:pointer;transition:opacity .2s,transform .15s;flex-shrink:0;white-space:nowrap; }
