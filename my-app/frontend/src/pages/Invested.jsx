...
const pendingToCompletedForm = (pendingRow) => ({
  ...
  amount:           pendingRow.amount_tobe_invested    || "",
              ...
              <Table size="small">
                <TableHead>
                  <TableRow sx={{ bgcolor: BLUE.light }}>
                    ...
                    {cols.map((col) => <TableCell key={col.key} sx={{ color: BLUE.text }}>{col.label}</TableCell>)}
                ...
                <TableBody>
                  {rows.length === 0 ? (
                    ...
                    rows.map((row, idx) => (
                      <TableRow key={row.id} sx={{ "&:hover": { bgcolor: BLUE.light }, "&:last-child td": { borderBottom: 0 } }}>
