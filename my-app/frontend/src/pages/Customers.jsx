...
// Prospects (customers_interested) columns
...
// Auto-fill mappings: customers → gift city inactive
...
// Auto-fill mappings: customers → prospects
...
function Snack({ msg, severity, onClose }) {
  ...
  return createPortal(
    <div style={{
      ...
      background: isSuccess ? "linear-gradient(135deg,#057a52,#0f9e6e)" : "linear-gradient(135deg,#a82424,#d14040)",
      color: "#fff",
    }}>
      {msg}
    </div>,
...
// ── Transfer Popover (mini menu: Gift City / Prospects) ──
function TransferPopover({ anchor, onSelect, onClose, theme = "dark" }) {
  ...
  return createPortal(
    <div ref={popRef} style={{
      ...
      borderRadius: 12, boxShadow: isDark
      ...
      <div style={{
        ...
        letterSpacing: ".1em", color: isDark ? "rgba(120,150,200,0.6)" : "rgba(0,0,0,0.4)",
...
export default function Customers({ inline = false, onDataChange, theme = "dark" }) {
  ...
  // Transfer dialogs
  const [transferType,   setTransferType]   = useState(null); // "giftcity" | "prospects"
  ...
  // User selected gift city or prospects from popover
  const handleTransferSelect = (type) => {
    setPopoverAnchor(null);
    setTransferType(type);
    if (type === "giftcity") {
      setTransferForm(toGiftCityInactive(popoverRow));
    } else {
      setTransferForm(toProspects(popoverRow));
    }
  };
  ...
  const saveTransfer = async () => {
    ...
    try {
      let url, payload;
      if (transferType === "giftcity") {
        url     = `${API}/gift-city/inactive`;
  ...
  const dangerStyle = { ...primaryStyle, background: "linear-gradient(135deg,#EF4444,#DC2626)", boxShadow: "0 4px 16px rgba(239,68,68,0.3)" };
  ...
  return (
    <div className={`mod-wrap${isDark ? "" : " theme-light"}`}>
      ...
      {/* ── Transfer Dialog (Gift City / Prospects) ── */}
          ...
          {transferCols.map(c => {
            ...
            return (
              <div key={c.key}>
                <CustField
                  col={c}
                  value={transferForm[c.key]}
                  onChange={setTransferField}
                  theme={theme}
                  readOnly={isAuto}
                />
                {isAuto ? (
                  <div style={{ fontSize: ".67rem", marginTop: 3, marginLeft: 2, color: "#10B981", fontWeight: 600 }}>
                    ✅ Auto-filled
                  </div>
                ) : (
                  <div style={{ fontSize: ".67rem", marginTop: 3, marginLeft: 2, color: transferColor, fontWeight: 600 }}>
                    ✏️ Fill manually
                  </div>
                )}
              </div>
      ...
      {confirm && renderDialog(
        ...
        "#EF4444",
        <p style={{ color: isDark ? "rgba(155,180,255,0.7)" : "rgba(0,0,0,0.6)", fontSize: ".84rem", lineHeight: 1.6, margin: 0 }}>
