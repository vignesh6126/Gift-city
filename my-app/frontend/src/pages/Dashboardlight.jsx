...
/* ─── stat card config (light accent colours) ────────────── */
const STATS = [
    {
        key: "empanelment_done", label: "Empanelment Done", sub: "this month",
        Icon: IcoEmpanel, iconBg: "rgba(42,109,217,0.15)", accent: "#2a6dd9", type: "ratio",
    },
    {
        key: "customers_onboarding", label: "Customers Onboarding", sub: "this week",
        Icon: IcoClients, iconBg: "rgba(15,158,110,0.15)", accent: "#0f9e6e", type: "ratio",
    },
    {
        key: "gift_city_ac_active", label: "Gift City A/C", sub: "prospects",
        Icon: IcoGift, iconBg: "rgba(201,124,8,0.15)", accent: "#c97c08", type: "ratio",
    },
    {
        key: "prospects_count", label: "Prospects", sub: "Prospects",
        Icon: IcoProspects, iconBg: "rgba(107,78,198,0.15)", accent: "#6b4ec6", type: "single",
    },
];
...
function StatCard({ cfg, data, loading }) {
    ...
    const single = typeof data === "number"
        ? data
        : (data?.total ?? data?.count ?? data?.completed ?? 0);
...
/* ═══════════════════════════════════════════════════
   SKY BLUE (LIGHT) THEME CSS — Navy Blue Borders
═══════════════════════════════════════════════════ */
const CSS = `
...
body{font-family:var(--fb);background:transparent;color:#111827;overflow-x:hidden;min-height:100%;margin:0;padding:12px;box-sizing:border-box}
...
.page-card{display:flex;flex:1;gap:10px;align-items:stretch;border-radius:20px;border:1px solid rgba(10,30,100,0.35);box-shadow:0 0 40px rgba(10,30,100,0.1);padding:10px;background:transparent}
...
.menu-btn{display:none;background:none;border:none;color:#111827;cursor:pointer;padding:5px;border-radius:7px}
.topbar-title{flex:1;font-weight:800;font-size:1.1rem;color:#111827;letter-spacing:.06em}
.user-chip{display:flex;align-items:center;gap:8px;padding:4px 16px 4px 4px;background:transparent;border:1px solid rgba(10,30,100,0.25);border-radius:40px}
.user-av{width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#2a6dd9,#6b4ec6);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:.85rem;color:#fff}
.user-name{font-size:.85rem;font-weight:700;color:#111827;letter-spacing:.02em}

.content{flex:1;padding:0 16px 28px;display:flex;flex-direction:column;gap:16px}
.stats-card{border:1px solid rgba(10,30,100,0.28);border-radius:20px;padding:14px;background:rgba(135,206,250,0.18);margin:0 4px}
.stats-row{display:grid;grid-template-columns:repeat(4,1fr);gap:14px}

.sc{position:relative;overflow:hidden;border-radius:16px;border:1px solid rgba(10,30,100,0.28);padding:12px 16px;background:rgba(135,206,250,0.22)!important;backdrop-filter:blur(12px);box-shadow:0 2px 12px rgba(10,30,100,0.08),inset 0 1px 0 rgba(255,255,255,0.5);transition:transform .35s var(--spring),box-shadow .35s;cursor:default}
.sc:hover{transform:translateY(-5px) scale(1.018);box-shadow:0 10px 32px rgba(10,30,100,0.15);background:rgba(135,206,250,0.32)!important}
.sc-body{position:relative;z-index:1;display:flex;align-items:center;gap:10px}
.sc-icon{width:40px;height:40px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;color:var(--accent);border:1.5px solid rgba(10,30,100,0.15)}
.sc-text{display:flex;flex-direction:column;gap:1px;min-width:0}
.sc-label{font-size:.55rem;font-weight:700;letter-spacing:.06em;color:rgba(0,0,0,0.65);white-space:normal;word-break:break-word;text-transform:uppercase;line-height:1.3}
.sc-ratio{display:flex;align-items:baseline;gap:2px}
.sc-big{font-size:1.75rem;font-weight:800;line-height:1;color:#111827}
.sc-slash{font-size:.9rem;color:rgba(0,0,0,0.45)}
.sc-tot{font-size:.9rem;color:rgba(0,0,0,0.55)}
.sc-single{font-size:2rem;font-weight:800;line-height:1;color:#111827}
.sc-sub{font-size:.6rem;color:var(--accent);margin-top:2px;font-weight:600}
.sc-skel{height:26px;width:70px;border-radius:6px;background:rgba(10,30,100,0.08);animation:sk 1.4s infinite}
...
.side-card{border-radius:16px;border:1px solid rgba(10,30,100,0.28);padding:16px 14px 14px;display:flex;flex-direction:column;background:transparent;animation:pIn .38s var(--spring) both;transition:transform .28s var(--spring),box-shadow .28s;position:relative;overflow:hidden}
.side-card::before{content:'';position:absolute;inset:0;border-radius:16px;opacity:.02;pointer-events:none}
.side-card-red{border-color:rgba(10,30,100,0.35)}
.side-card-red::before{background:radial-gradient(ellipse at top left,#e05555,transparent 70%)}
.side-card-purple{border-color:rgba(10,30,100,0.35)}
.side-card-purple::before{background:radial-gradient(ellipse at top left,#7b5ef8,transparent 70%)}
...
.side-card-label{font-size:.5rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:rgba(0,0,0,0.6);line-height:1.2;flex:1}
.side-card-count{font-size:1.45rem;font-weight:800;line-height:1;margin-left:auto;flex-shrink:0}
...
.side-card-row-lbl{color:rgba(0,0,0,0.6);flex:1;font-weight:500;font-size:.7rem}
...
.module-panel{background:transparent;border:1px solid rgba(10,30,100,0.28);border-radius:18px;overflow:visible;box-shadow:none;animation:pIn .38s var(--spring) both;margin:0 4px 4px}
@keyframes pIn{from{opacity:0;transform:translateY(14px) scale(.985)}to{opacity:1;transform:none}}

.mod-wrap{min-height:200px}
.mod-hdr{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;padding:14px 20px;border-bottom:1px solid rgba(10,30,100,0.15)}
.tabs-row{display:flex;gap:6px;flex-wrap:wrap}
.tab-pill{padding:7px 20px;border-radius:50px;border:1px solid rgba(10,30,100,0.2);background:transparent;color:rgba(0,0,0,0.6);font-size:.82rem;font-weight:600;cursor:pointer;font-family:var(--fh);transition:all .28s var(--spring);white-space:nowrap;letter-spacing:.05em}
.tab-pill:hover{color:#000;border-color:rgba(10,30,100,0.35);background:rgba(10,30,100,0.05)}
.tab-active{background:rgba(42,109,217,0.12)!important;color:#000!important;border-color:rgba(42,109,217,0.4)!important;box-shadow:none;transform:translateY(-1px)}
.tbl-hdr{display:flex;align-items:center;gap:10px;padding:12px 20px;border-bottom:1px solid rgba(10,30,100,0.15)}
.tbl-title{font-weight:800;font-size:1rem;color:#111827;letter-spacing:.06em}
...
.fd-tbl th{padding:13px 18px;text-align:left;font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:rgba(0,0,0,0.6);border-bottom:1px solid rgba(10,30,100,0.25);white-space:nowrap}
...
.fd-tbl td{padding:8px 18px;color:#111827;border-bottom:1px solid rgba(10,30,100,0.15);vertical-align:middle;white-space:nowrap;background:transparent;font-size:.86rem}
.fd-num{color:rgba(0,0,0,0.4);font-size:.74rem}
.fd-empty{text-align:center;padding:56px 20px!important;color:rgba(0,0,0,0.4);font-style:italic;font-size:.84rem}
...
.add-btn{display:flex;align-items:center;gap:6px;padding:8px 16px;border-radius:10px;border:none;cursor:pointer;color:#fff;font-size:.8rem;font-weight:700;background:linear-gradient(135deg,#2a6dd9,#5a3fb5);box-shadow:0 4px 14px rgba(42,109,217,0.32);transition:all .28s var(--spring);letter-spacing:.03em}
...
.spinner{width:30px;height:30px;border-radius:50%;border:3px solid rgba(42,109,217,0.18);border-top-color:#2a6dd9;animation:spin .8s linear infinite}
@keyframes spin{to{transform:rotate(360deg)}}
.dlg-ov{position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,0.35);backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;padding:16px;animation:fIn .18s ease}
@keyframes fIn{from{opacity:0}to{opacity:1}}
.dlg-box{background:rgba(255,255,255,0.7);backdrop-filter:blur(40px) saturate(180%);border:1px solid rgba(10,30,100,0.2);border-radius:18px;width:100%;max-width:450px;overflow:hidden;box-shadow:0 8px 40px rgba(10,30,100,0.15);animation:dPop .32s var(--spring)}
@keyframes dPop{from{opacity:0;transform:scale(.88) translateY(22px)}to{opacity:1;transform:none}}
.dlg-hdr{padding:18px 22px 14px;border-bottom:1px solid rgba(10,30,100,0.15);display:flex;align-items:center;gap:11px;background:transparent}
.dlg-bar{width:4px;height:22px;border-radius:2px;flex-shrink:0}
.dlg-ttl{font-weight:800;font-size:1rem;color:#111827}
.dlg-sub{font-size:.67rem;color:rgba(0,0,0,0.5);margin-top:2px}
.dlg-body{padding:18px 22px;display:flex;flex-direction:column;gap:12px;max-height:45vh;overflow-y:auto;background:transparent}
.dlg-foot{padding:14px 22px;border-top:1px solid rgba(10,30,100,0.12);display:flex;justify-content:flex-end;gap:8px;background:transparent}
.fld{display:flex;flex-direction:column;gap:6px}
.fld-lbl{font-size:.67rem;font-weight:700;color:rgba(0,0,0,0.65);letter-spacing:.08em;text-transform:uppercase}
.fld-inp{width:100%;padding:9px 12px;border-radius:10px;background:rgba(255,255,255,0.5);border:1px solid rgba(10,30,100,0.2);color:#111827;font-size:.84rem;font-family:var(--fb);transition:border-color .2s,box-shadow .2s;outline:none}
...
.btn-cancel{padding:8px 16px;border-radius:10px;border:1px solid rgba(10,30,100,0.2);background:none;color:rgba(0,0,0,0.65);cursor:pointer;font-size:.8rem;font-weight:600;transition:all .2s}
...
.theme-switch-thumb{position:absolute;top:3px;left:3px;width:16px;height:16px;border-radius:50%;background:linear-gradient(135deg,#f7d46a,#ffb020);box-shadow:0 1px 6px rgba(200,140,0,0.5);transition:transform .3s var(--spring);transform:translateX(20px)}
...
@media(max-width:768px){
  .root{padding:8px}
  .side{position:fixed;top:8px;left:8px;bottom:8px;border-radius:20px;transform:translateX(calc(-100% - 20px));background:rgba(255,255,255,0.2);backdrop-filter:blur(24px)}
